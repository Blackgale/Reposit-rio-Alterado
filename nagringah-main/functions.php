...
###Adicionou até corretamente, mas está faltando coisa
add_action( 'woocommerce_thankyou', 'bbloomer_redirectcustom');
function bbloomer_redirectcustom( $order_id ){
    $order = wc_get_order( $order_id );
    $url = 'https://nagringah.com.br/meus-pedidos/';
    wp_safe_redirect( $url );
    exit;
}

/************
CRIAÇÃO DE SHORTECODE
*************/
...
function getAvatar(){
	...
	if($params['uid']){
		return get_avatar($params['uid'], 120);
	}else{
		$html = get_avatar(get_current_user_id(), 200, array(200, 200));
		$html  .= '<div class="pm-bg-dk pg-profile-change-img">
			<div id="pm-change-image" class="pg-item-image-change">';
				$html .= "Atualizar imagem";
			$html  .= '</div>
			</div>';
		return $html;
	}
}
add_shortcode('get-avatar', 'getAvatar'); 
...
///LINHA ALTERADA
function redirect_to_login( $url ) {
...
add_shortcode('count-cart', 'countCart'); 
...
add_shortcode('count-order', 'countOrder'); 

//back button
function backbutton(){
	ob_start();
    get_template_part('templates/profile/back');
    return ob_get_clean(); 
}
add_shortcode('back-button', 'backbutton'); 

//request a friend
function requestFriend(){
	ob_start();
    get_template_part('templates/profile/request');
    return ob_get_clean(); 
}
add_shortcode('request-friend', 'requestFriend'); 
...
add_shortcode('get-name', 'getName'); 

//getlist of user by id
function getWhishlistByUser(){
	ob_start();
    get_template_part('templates/wishlist/wishlist');
    return ob_get_clean(); 
}
add_shortcode('get-whishlist', 'getWhishlistByUser'); 

//getlist of user by id
function getDetailByUser(){
	ob_start();
    get_template_part('templates/profile/about');
    return ob_get_clean(); 
}
add_shortcode('get-detail', 'getDetailByUser'); 

//getlist of user by code
function getProductWhishlist(){
	ob_start();
    get_template_part('templates/wishlist/product');
    return ob_get_clean(); 
}
add_shortcode('get-productwhishlist', 'getProductWhishlist'); 
...
add_shortcode('button_header', 'buttonHeader'); 
...
add_shortcode('menu-header', 'menuHeader'); 

//menu list interno
function menuSidebar(){
	ob_start();
    get_template_part('templates/sidebar/list');
    return ob_get_clean(); 
}
add_shortcode('menu-sidebar', 'menuSidebar'); 

remove_filter('logout_redirect', 'get_home_url');


//getlist of user by id
function menuFooterMobile(){
	ob_start();
    get_template_part('templates/footer/mobile');
    return ob_get_clean(); 
}
add_shortcode('menuFooterMobile', 'menuFooterMobile'); 
...
function frete(){
	ob_start();
    get_template_part('templates/frete/calcula');
    return ob_get_clean(); 
}
add_shortcode('frete', 'frete'); 
...
if ( ! function_exists( 'et_divi_customizer_theme_settings' ) ) :
	...
	function et_divi_customizer_theme_settings( $wp_customize ) {
		...
		###Arrumar responsivo de 72 horas front
		$wp_customize->add_control(
			new ET_Divi_Range_Option(
				$wp_customize,
				'et_divi[phone_header_font_size]',
